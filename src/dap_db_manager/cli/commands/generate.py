...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    # Extract DAP root for cross-compilation (if provided)
    dap_root = None
    if hasattr(args, "dap_root") and args.dap_root:
        dap_root = str(args.dap_root.resolve())
        logging.info("Using DAP root for path translation: %s", dap_root)

        # Validate that music_path is under dap_root
        if not str(music_path).startswith(dap_root):
            if use_json:
                json_output(
                    ErrorResponse(
                        error="invalid_input",
                        message=f"Music directory ({music_path}) must be under DAP root ({dap_root})",
                    ),
                    ExitCode.INVALID_INPUT,
                )
            logging.error(
                "Music directory (%s) must be under DAP root (%s)",
                music_path,
                dap_root,
            )
            sys.exit(ExitCode.INVALID_INPUT)
    ...
    if not config.is_mount_notation_configured():
        device_path = dap_root if dap_root else args.output
        if not use_json:
    ...
    try:
        ...
        logging.exception("Database generation failed: %s", e)
