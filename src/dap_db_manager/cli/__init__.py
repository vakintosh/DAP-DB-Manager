"""Command-line interface for DAP DB Manager (ddm).

This package provides the 'ddm' command-line tool with various subcommands:
...
def parse_args() -> tuple[argparse.ArgumentParser, argparse.Namespace]:
    ...
    parser = argparse.ArgumentParser(
        prog="ddm",
        usage="ddm <command> [options]",
        description="DAP DB Manager - Generate and manage Rockbox database files",
    ...
    generate_parser = subparsers.add_parser(
        "generate",
        help="Generate Rockbox database from music folder (supports cross-compilation with --dap-root)",
        usage="ddm generate --music-dir <path/to/source/music/dir> --output <path/to/target/database/dir> [options]",
        description="Scan music folder and generate Rockbox database files. Supports cross-compilation with --dap-root for 180x faster database generation on laptop/server instead of on device.",
    ...
    generate_options.add_argument(
        ...
        metavar="PATH",
        help=(
            "DAP mount point for cross-compilation. When generating database on a laptop "
            "for a DAP, specify the mount point (e.g., /Volumes/DAP or E:) to strip from paths. "
            "Example: /Volumes/DAP/Music/Song.mp3 → /Music/Song.mp3 in database"
        ),
    )
    ...
    load_parser = subparsers.add_parser(
        ...
        usage="ddm load --db-dir <path/to/database/dir> [options]",
    ...
    validate_parser = subparsers.add_parser(
        ...
        usage="ddm validate --db-dir <path/to/database/dir> [options]",
    ...
    write_parser = subparsers.add_parser(
        ...
        usage="ddm write --db-dir <path/to/source/database/dir> --output <path/to/destination/dir> [options]",
    ...
    inspect_parser = subparsers.add_parser(
        ...
        usage="ddm inspect --db-dir <path/to/database/dir> [--file-number N] [options]",
        description=(
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update database with new/deleted/renamed files (supports cross-compilation with --dap-root)",
        usage="ddm update --db-dir <path/to/database> --music-dir <path/to/music> [options]",
        description="Incrementally update existing database: scans for new files, detects renamed/moved files to preserve statistics, marks missing files as deleted. Faster than full rebuild. Supports cross-compilation with --dap-root (180x faster on laptop/server). Similar to Rockbox's 'Update Now' feature.",
        formatter_class=RichHelpFormatter,
        add_help=False,
    )
    ...
    update_options.add_argument(
        ...
        metavar="PATH",
        help=(
            "DAP mount point for cross-compilation. When updating database on a laptop "
            "for a DAP, specify the mount point (e.g., /Volumes/DAP or E:) to strip from paths. "
            "Example: /Volumes/DAP/Music/Song.mp3 → /Music/Song.mp3 in database"
        ),
    )
